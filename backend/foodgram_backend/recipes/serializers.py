...
class IngredientInRecipeGetSerializer(serializers.Serializer):
    ...
    """Сериализатор ингредиентов, используемых в рецепте."""
...
class ShortLinkSerializer(serializers.ModelSerializer):
    ...
    def get_short_link_in_response(self, obj):
        ...
        obj_with_path, created = ShortLink.objects.get_or_create(
            recipe=obj,
            original_path=original_path)
        if not obj_with_path.short_code_path or created:
            self.create_short_url(obj_with_path, request)
        short_absolute_uri = request.build_absolute_uri(
            f'/s/{obj_with_path.short_code_path}/'
        )
